		...
		try {
			...
			if (!group.members.includes(userId)) {
				throw new ValidationError(
					"You must be a member of the group to review other members"
				);
			}

			// Get eligible users for review
			const eligibleUsers = await reviewData.getEligibleReviewUsers(
				groupId,
				userId
			);
			...
			if (eligibleUsers.length === 0) {
				return res.render("review-form", {
					...
					message:
						"You have already reviewed all members of this group.",
				});
			}
		...
		try {
			...
			if (
				...
				try {
					...
					const group = await groupData.getGroupById(groupId);
					const eligibleUsers =
						await reviewData.getEligibleReviewUsers(
							groupId,
							req.session.user.id
						);
		...
		try {
			...
			if (
				...
				try {
					const reviewId = req.params.id;
					const review = await reviewData.getReviewById(reviewId);
					const targetUser = await userData.findUserById(
						review.targetId
					);
					const group = await groupData.getGroupById(review.groupId);
...
router.get(
	...
	asyncHandler(async (req, res) => {
		try {
			...
			// Calculate most active reviewer
			const reviewerCounts = {};
			groupReviews.forEach((review) => {
				const reviewerId = review.reviewerId;
				reviewerCounts[reviewerId] =
					(reviewerCounts[reviewerId] || 0) + 1;
			});

			let mostActiveReviewerId = null;
			let mostReviews = 0;

			Object.entries(reviewerCounts).forEach(([reviewerId, count]) => {
				if (count > mostReviews) {
					mostActiveReviewerId = reviewerId;
					mostReviews = count;
				}
			});

			const mostActiveReviewer =
				mostActiveReviewerId && users[mostActiveReviewerId]
					? users[mostActiveReviewerId].userName
					: "N/A";
