...
export const createReview = async (
	...
	try {
		...
		if (existingReview) {
			throw new ValidationError(
				"You have already reviewed this user for this group"
			);
		}
		...
		const createdReview = await getReviewById(
...
export const getReviewsByGroup = async (groupId) => {
	try {
		...
		const reviewsCollection = await reviews();
		const groupReviews = await reviewsCollection
			.find({
				groupId: new ObjectId(groupId),
			})
			.toArray();
...
export const updateUserRating = async (userId) => {
	try {
		...
		// Check if user exists, but don't throw error if rating unchanged
		if (updateResult.matchedCount === 0) {
			console.error(
				`User with ID ${userId} not found when updating rating`
			);
		}
...
export const getEligibleReviewUsers = async (groupId, userId) => {
	try {
		...
		const reviewedUserIds = userReviews.map((review) =>
			review.targetId.toString()
		);

		// Filter out the current user and already reviewed users
		const eligibleUsers = group.members.filter(
			(memberId) =>
		...
		const usersCollection = await users();
		const eligibleUserDetails = await usersCollection
			.find({
